...
# Shiro Explorer optimization cache. Keeps directory scans fast while avoiding
# long-lived stale results after files are generated/moved.
shiro_scan_cache = {}
shiro_scan_cache_ttl = 8.0
shiro_scan_cache_max_entries = 64
shiro_explorer_max_files = 12000
...
shiro_favorite_tab_name = "New Favorite"
shiro_explorer_tab_name = "Explorer"
shiro_default_sources = ["txt2img", "img2img", "Extras"]
shiro_all_default_sources = "All default sources"
default_tab_options = ["Recent T2I", "Recent I2I", shiro_explorer_tab_name, shiro_favorite_tab_name]

def check_image_browser_active_tabs():
    old_default_tabs = "txt2img, img2img, txt2img-grids, img2img-grids, Extras, Favorites, Others, All, Maintenance"
    if hasattr(opts, "image_browser_active_tabs"):
        active_tabs = str(opts.image_browser_active_tabs).strip()
        if active_tabs == "" or active_tabs == old_default_tabs:
            shared.opts.__setattr__("image_browser_active_tabs", ", ".join(default_tab_options))
            shared.opts.save(shared.config_filename)

    last_default_tab = wib_db.get_last_default_tab()
    if last_default_tab[0] in ("Others", "Maintenance"):
        with wib_db.transaction() as cursor:
            wib_db.update_db_data(cursor, "last_default_tab", shiro_favorite_tab_name)
...
path_maps = {
    ...
    "Recent T2I": opts.outdir_samples or opts.outdir_txt2img_samples,
    "Recent I2I": opts.outdir_samples or opts.outdir_img2img_samples,
    shiro_explorer_tab_name: opts.outdir_samples or opts.outdir_txt2img_samples,
...
def img_path_subdirs_list(img_path):
    subdirs = [none_select]
    try:
        img_path, _ = pure_path(img_path)
        if not img_path or img_path == none_select or not os.path.isdir(img_path):
            return subdirs
        with os.scandir(img_path) as entries:
            dirs = [entry.path for entry in entries if entry.is_dir(follow_symlinks=False)]
        subdirs.extend(sorted(dirs, key=lambda x: natural_keys(os.path.basename(x).lower())))
    except Exception as e:
        logger.warning(f"Image Browser New Favorite: failed reading subdirectories for {img_path}: {e}")
    return subdirs

def img_path_subdirs_get(img_path):
    subdirs = img_path_subdirs_list(img_path)
    return gr.update(choices=subdirs, value=none_select)

def shiro_subdir_update(img_path):
    subdirs = img_path_subdirs_list(img_path)
    visible = len(subdirs) > 1
    return gr.update(choices=subdirs, value=none_select, visible=visible), gr.update(visible=visible)

def shiro_subdir_reset():
    return gr.update(choices=[none_select], value=none_select, visible=False), gr.update(visible=False)

def shiro_favorite_root_change(img_dir, path_recorder, load_switch, img_path_browser, img_path_depth):
    # Changing the root favorite must always load the root directory and reset nested subfolder state.
    outputs = change_dir_dropdown(img_dir, path_recorder, load_switch, img_path_browser, img_path_depth)
    img_path_value = outputs[5]
    sub1, row1 = shiro_subdir_update(img_path_value)
    sub2, row2 = shiro_subdir_reset()
    sub3, row3 = shiro_subdir_reset()
    return (*outputs, sub1, row1, sub2, row2, sub3, row3)

def shiro_favorite_auto_open_once(already_opened, img_dir, path_recorder, load_switch, img_path_browser, img_path_depth):
    # Gradio can keep the New Favorite path stuck on a subdirectory between UI events.
    # When the tab is opened, run Open root once per browser session so the first
    # saved folder becomes the real active directory. Subsequent tab visits keep the
    # user's current navigation state.
    if already_opened:
        return (*([gr.update()] * 13), True)
    if not img_path_browser or img_path_browser == none_select:
        return (*([gr.update()] * 13), True)
    try:
        outputs = shiro_favorite_root_change(img_dir, path_recorder, load_switch, img_path_browser, img_path_depth)
        return (*outputs, True)
    except Exception as e:
        logger.warning(f"Image Browser New Favorite: auto Open root failed: {e}")
        return (*([gr.update()] * 13), True)

def shiro_favorite_textbox_change(img_dir, path_recorder, load_switch, img_path_browser, img_path_depth):
    outputs = change_dir_textbox(img_dir, path_recorder, load_switch, img_path_browser, img_path_depth)
    img_path_value = outputs[5]
    sub1, row1 = shiro_subdir_update(img_path_value)
    sub2, row2 = shiro_subdir_reset()
    sub3, row3 = shiro_subdir_reset()
...
def shiro_favorite_subdir_change(img_dir, path_recorder, load_switch, img_path_browser, img_path_depth):
    outputs = change_dir_dropdown(img_dir, path_recorder, load_switch, img_path_browser, img_path_depth)
    img_path_value = outputs[5]
    next_sub, next_row = shiro_subdir_update(img_path_value)
    lower_sub, lower_row = shiro_subdir_reset()
    return (*outputs, next_sub, next_row, lower_sub, lower_row)

def shiro_favorite_subdir_change_level2(img_dir, path_recorder, load_switch, img_path_browser, img_path_depth):
    outputs = change_dir_dropdown(img_dir, path_recorder, load_switch, img_path_browser, img_path_depth)
    img_path_value = outputs[5]
    next_sub, next_row = shiro_subdir_update(img_path_value)
    return (*outputs, next_sub, next_row)
...
def shiro_explorer_source_choices(path_recorder_formatted=None):
    path_recorder_formatted = path_recorder_formatted or []
    # Keep the Explorer conservative: choose one source at a time.
    # The combined default scan was too expensive on large output folders.
    return shiro_default_sources + path_recorder_formatted

def shiro_source_to_path(source):
    if source in path_maps:
        # Built-in output folders are commonly organized with date/subfolders.
        # Main mode needs a small default depth so filters can see nested files;
        # Raw mode still forces depth 0 inside get_image_page.
        return os.path.realpath(path_maps[source]), 2
    if source == shiro_all_default_sources:
        return os.path.realpath(path_maps.get("txt2img", "")), -1
    return pure_path(source)

def shiro_year_choices():
    current_year = datetime.now().year
    return ["All"] + [str(year) for year in range(current_year + 1, 2014, -1)]

def shiro_filter_int(value):
    """Normalize Gradio dropdown/number values used by the Explorer date filter.
    Accepts strings like "2026", "04", numeric values, and float-like UI values.
    Returns None for All/empty/invalid values so partial filters work reliably.
    """
    try:
        if value is None:
            return None
        text = str(value).strip()
        if text == "" or text.lower() in ("all", "none", str(none_select).lower()):
            return None
        return int(float(text))
    except Exception:
        return None

def shiro_normalize_date_filter(year="All", month="All", day="All"):
    return (shiro_filter_int(year), shiro_filter_int(month), shiro_filter_int(day))

def shiro_date_match(fstat, year="All", month="All", day="All"):
    try:
        dt = datetime.fromtimestamp(fstat.st_mtime)
        y, m, d = shiro_normalize_date_filter(year, month, day)
        # Each field is independent. This makes partial filters valid:
        # Year only, Year+Month, or Year+Month+Day.
        ...
        logger.warning(f"Image Browser Explorer: date filter failed; keeping file visible: {e}")
        return True

def shiro_explorer_change_source(source):
    img_path, img_path_depth = shiro_source_to_path(source)
    if source == shiro_all_default_sources:
        img_path_depth = 0
    return img_path, img_path_depth

def shiro_explorer_change_source_with_subdirs(source):
    img_path, img_path_depth = shiro_explorer_change_source(source)
    sub1, row1 = shiro_subdir_update(img_path)
    sub2, row2 = shiro_subdir_reset()
    sub3, row3 = shiro_subdir_reset()
    return img_path, img_path_depth, sub1, row1, sub2, row2, sub3, row3

def shiro_explorer_add_remove(img_dir, path_recorder, add_remove, img_path_depth):
    value = "txt2img"

    if add_remove == "add":
        if not img_dir or not os.path.isdir(os.path.realpath(img_dir)):
            formatted = [value.get("path_display") for key, value in path_recorder.items()]
            formatted = sorted(formatted, key=lambda x: natural_keys(x.lower()))
            return path_recorder, gr.update(choices=shiro_explorer_source_choices(formatted), value="txt2img")
        path_recorder, dropdown_update = img_path_add_remove(img_dir, path_recorder, add_remove, img_path_depth)
        try:
            value = dropdown_update.get("value", value)
        except Exception:
            pass
    elif add_remove == "remove":
        remove_path, _ = pure_path(img_dir)
        if remove_path in path_recorder:
            path_recorder, dropdown_update = img_path_add_remove(img_dir, path_recorder, add_remove, img_path_depth)
            try:
                value = dropdown_update.get("value", value)
            except Exception:
    ...
    formatted = [value.get("path_display") for key, value in path_recorder.items()]
    formatted = sorted(formatted, key=lambda x: natural_keys(x.lower()))
    choices = shiro_explorer_source_choices(formatted)
    if value not in choices:
        value = "txt2img"
    return path_recorder, gr.update(choices=choices, value=value)

def shiro_increment_page(turn_page_switch):
    try:
        shiro_scan_cache.clear()
    except Exception:
        pass
    return 1, turn_page_switch + 1
...
def shiro_cache_get(key):
    try:
        item = shiro_scan_cache.get(key)
        if not item:
            return None
        ts, value = item
        if time.time() - ts > shiro_scan_cache_ttl:
            shiro_scan_cache.pop(key, None)
            return None
        return list(value)
    except Exception:
        return None

def shiro_cache_set(key, value):
    try:
        if len(shiro_scan_cache) >= shiro_scan_cache_max_entries:
            oldest_key = min(shiro_scan_cache, key=lambda k: shiro_scan_cache[k][0])
            shiro_scan_cache.pop(oldest_key, None)
        shiro_scan_cache[key] = (time.time(), list(value))
    except Exception:
        pass

def shiro_dir_scan_signature(curr_path):
    try:
...
def traverse_all_files(curr_path, image_list, tab_base_tag_box, img_path_depth, shiro_date_filter=None, shiro_max_files=None) -> List[Tuple[str, os.stat_result, str, int]]:
    logger.debug(f"curr_path: {curr_path}")
    if not curr_path:
        return image_list

    try:
        curr_path = os.path.realpath(curr_path)
        if not os.path.isdir(curr_path):
            logger.warning(f"Image Browser: skipped missing/non-directory path: {curr_path}")
            return image_list
    except (FileNotFoundError, PermissionError, OSError) as e:
        logger.warning(f"Image Browser: failed scanning {curr_path}: {e}")
        return image_list

    # Explorer should not inherit the global "with subdirs" option because that can
    # recursively scan huge output trees and look like an infinite search. For Explorer,
    # depth is explicit: 0 = current folder only, 1+ = limited recursion, -1 = all.
    is_explorer = tab_base_tag_box == "image_browser_tab_explorer"
    cache_key = None
    if is_explorer:
        normalized_filter = shiro_normalize_date_filter(*(shiro_date_filter or ("All", "All", "All")))
        cache_key = (curr_path, int(img_path_depth or 0), normalized_filter, shiro_dir_scan_signature(curr_path))
        cached = shiro_cache_get(cache_key)
        if cached is not None:
            image_list.extend(cached[:shiro_max_files] if shiro_max_files else cached)
            return image_list

    local_results = []

    def should_add_file(fstat):
        if shiro_date_filter is None:
            return True
        return shiro_date_match(fstat, *shiro_date_filter)

    def scan_dir(path, depth):
        if shiro_max_files and len(local_results) >= shiro_max_files:
            return
        try:
            with os.scandir(path) as it:
                ...
                for entry in it:
                    try:
                        entries.append((entry.path, entry.stat(follow_symlinks=False)))
                    except (FileNotFoundError, PermissionError, OSError) as e:
                        logger.warning(f"Image Browser: skipped inaccessible entry {getattr(entry, 'path', entry.name)}: {e}")
        except (FileNotFoundError, PermissionError, OSError) as e:
            logger.warning(f"Image Browser: failed scanning {path}: {e}")
            return

        # Newer files first for Explorer/date workflows. This improves perceived speed
        # because page 1 is usually what the user wants.
        if is_explorer:
            entries.sort(key=lambda x: x[1].st_mtime, reverse=True)

        for fname, fstat in entries:
            if shiro_max_files and len(local_results) >= shiro_max_files:
                return
            ext = os.path.splitext(fname)[1].lower()
            if ext in image_ext_list or ext in video_ext_list:
                if should_add_file(fstat):
                    local_results.append((fname, fstat))
            elif stat.S_ISDIR(fstat.st_mode):
                if is_explorer:
                    should_recurse = img_path_depth < 0 or depth < int(img_path_depth or 0)
                else:
                    should_recurse = (
                        (opts.image_browser_with_subdirs and tab_base_tag_box not in ("image_browser_tab_others", "image_browser_tab_newfavorite", "image_browser_tab_explorer"))
                        or (tab_base_tag_box == "image_browser_tab_all")
                        or (tab_base_tag_box in ("image_browser_tab_others", "image_browser_tab_newfavorite") and img_path_depth != 0 and (depth < img_path_depth or img_path_depth < 0))
                    )
                if should_recurse:
                    scan_dir(fname, depth + 1)

    scan_dir(curr_path, 0)

    if cache_key is not None:
        shiro_cache_set(cache_key, local_results)
    image_list.extend(local_results)
    return image_list
...
def get_all_images(dir_name, sort_by, sort_order, keyword, tab_base_tag_box, img_path_depth, ranking_filter, ranking_filter_min, ranking_filter_max, aes_filter_min, aes_filter_max, exif_keyword, negative_prompt_search, use_regex, explorer_mode="Raw", explorer_source="", explorer_year="All", explorer_month="All", explorer_day="All"):
    global current_depth
    logger.debug("get_all_images")
    current_depth = 0
    fileinfos = []
    if tab_base_tag_box == "image_browser_tab_explorer" and explorer_source == shiro_all_default_sources:
        # Safety fallback only: the combined source is no longer exposed in the UI.
        # Keep it non-recursive to avoid runaway scans on large output folders.
        for source in shiro_default_sources:
            path = path_maps.get(source, "")
            if not path:
                continue
            list1 = fileinfos
            list2 = traverse_all_files(os.path.realpath(path), [], tab_base_tag_box, 0)
            tmp = dict(list1)
            tmp.update(dict(list2))
            fileinfos = list(tmp.items())
    elif tab_base_tag_box == "image_browser_tab_all":
        ...
        shiro_filter = None
        shiro_limit = None
        if tab_base_tag_box == "image_browser_tab_explorer" and explorer_mode == "Main":
            shiro_filter = (explorer_year, explorer_month, explorer_day)
            shiro_limit = shiro_explorer_max_files
        elif tab_base_tag_box == "image_browser_tab_explorer" and explorer_mode == "Raw":
            shiro_limit = shiro_explorer_max_files
        fileinfos = traverse_all_files(dir_name, [], tab_base_tag_box, img_path_depth, shiro_filter, shiro_limit)

    if tab_base_tag_box == "image_browser_tab_explorer" and explorer_mode == "Main":
        # Date filtering is already applied during traversal for Explorer. Keep this
        # as a safe fallback in case older cached data is encountered. Partial filters
        # are valid here: Year only, Year+Month, or Year+Month+Day.
        if any(v is not None for v in shiro_normalize_date_filter(explorer_year, explorer_month, explorer_day)):
            fileinfos = [x for x in fileinfos if shiro_date_match(x[1], explorer_year, explorer_month, explorer_day)]
...
def get_image_page(img_path, page_index, filenames, keyword, sort_by, sort_order, tab_base_tag_box, img_path_depth, ranking_filter, ranking_filter_min, ranking_filter_max, aes_filter_min, aes_filter_max, exif_keyword, negative_prompt_search, use_regex, explorer_mode="Raw", explorer_source="", explorer_year="All", explorer_month="All", explorer_day="All"):
    ...
    img_path, _ = pure_path(img_path)
    if tab_base_tag_box == "image_browser_tab_explorer" and explorer_mode == "Raw":
        img_path_depth = 0
    filenames = get_all_images(img_path, sort_by, sort_order, keyword, tab_base_tag_box, img_path_depth, ranking_filter, ranking_filter_min, ranking_filter_max, aes_filter_min, aes_filter_max, exif_keyword, negative_prompt_search, use_regex, explorer_mode, explorer_source, explorer_year, explorer_month, explorer_day)
...
def create_tab(tab: ImageBrowserTab, current_gr_tab: gr.Tab):
    ...
    explorer_dir = False
    shiro_first_favorite = path_recorder_formatted[0] if path_recorder_formatted else None
    if tab.name in ("Others", shiro_favorite_tab_name):
        others_dir = True
        standard_ui = False
        if shiro_first_favorite:
            dir_name, shiro_first_depth = pure_path(shiro_first_favorite)
        else:
            dir_name = ""
            shiro_first_depth = 0
    elif tab.name == shiro_explorer_tab_name:
        explorer_dir = True
        standard_ui = True
        dir_name = path_maps.get("txt2img", tab.path)
    ...
    shiro_initial_subdirs = img_path_subdirs_list(dir_name) if (others_dir or explorer_dir) else [none_select]
    shiro_initial_subdirs_visible = (others_dir or explorer_dir) and len(shiro_initial_subdirs) > 1
    ...
    with gr.Row():                 
        ...
        shiro_favorite_auto_opened = gr.State(False)
    ...
    with gr.Row(visible=others_dir, elem_classes="shiro-favorite-align-row"):
        with gr.Column(scale=10):
            img_path = gr.Textbox(dir_name, label="New Favorite directory", placeholder="Input images directory", interactive=others_dir)
        with gr.Column(scale=1):
            img_path_depth = gr.Number(value=(shiro_first_depth if others_dir else "0"), label="Sub directory depth")
        with gr.Column(scale=1, elem_classes="shiro-favorite-align-button-col"):
            gr.HTML("<div class='shiro-favorite-align-spacer'></div>")
            img_path_save_button = gr.Button(value="Add to / replace in New Favorite")

    with gr.Row(visible=others_dir, elem_classes="shiro-favorite-align-row"):
        with gr.Column(scale=9):
            img_path_browser = gr.Dropdown(choices=path_recorder_formatted, value=shiro_first_favorite, label="New Favorite folders")
        with gr.Column(scale=1, elem_classes="shiro-favorite-align-button-col"):
            gr.HTML("<div class='shiro-favorite-align-spacer'></div>")
            img_path_root_button = gr.Button(value="Open root")
        with gr.Column(scale=1, elem_classes="shiro-favorite-align-button-col"):
            gr.HTML("<div class='shiro-favorite-align-spacer'></div>")
            img_path_remove_button = gr.Button(value="Remove from New Favorite")

    with gr.Row(visible=shiro_initial_subdirs_visible, elem_classes="shiro-favorite-align-row") as img_path_subdirs_row:
        with gr.Column(scale=10):
            img_path_subdirs = gr.Dropdown(choices=shiro_initial_subdirs, value=none_select, label="Sub directories", interactive=True, elem_id=f"{tab.base_tag}_img_path_subdirs")
        with gr.Column(scale=1, elem_classes="shiro-favorite-align-button-col"):
            gr.HTML("<div class='shiro-favorite-align-spacer'></div>")
            img_path_subdirs_button = gr.Button(value="Refresh")

    with gr.Row(visible=False, elem_classes="shiro-favorite-align-row") as img_path_subdirs_level2_row:
        with gr.Column(scale=10):
            img_path_subdirs_level2 = gr.Dropdown(choices=[none_select], value=none_select, label="Sub directories level 2", interactive=True, elem_id=f"{tab.base_tag}_img_path_subdirs_level2")
        with gr.Column(scale=1, elem_classes="shiro-favorite-align-button-col"):
            gr.HTML("<div class='shiro-favorite-align-spacer'></div>")
            img_path_subdirs_level2_button = gr.Button(value="Refresh")

    with gr.Row(visible=False, elem_classes="shiro-favorite-align-row") as img_path_subdirs_level3_row:
        with gr.Column(scale=10):
            img_path_subdirs_level3 = gr.Dropdown(choices=[none_select], value=none_select, label="Sub directories level 3", interactive=True, elem_id=f"{tab.base_tag}_img_path_subdirs_level3")
        with gr.Column(scale=1, elem_classes="shiro-favorite-align-button-col"):
            gr.HTML("<div class='shiro-favorite-align-spacer'></div>")
            img_path_subdirs_level3_button = gr.Button(value="Refresh")
    
    with gr.Box(visible=explorer_dir):
        with gr.Row():
            with gr.Column(scale=2):
                explorer_mode = gr.Radio(value="Main", choices=["Main", "Raw"], label="Explorer mode", interactive=True)
            with gr.Column(scale=5):
                explorer_source = gr.Dropdown(choices=shiro_explorer_source_choices(path_recorder_formatted), value="txt2img", label="Source folder", interactive=True)
            with gr.Column(scale=1):
                explorer_refresh = gr.Button(value="Search / Refresh")
        with gr.Row():
            with gr.Column(scale=6):
                explorer_manual_path = gr.Textbox(value="", label="Manual folder path")
            with gr.Column(scale=1):
                explorer_manual_depth = gr.Number(value="0", label="Depth")
            with gr.Column(scale=1):
                explorer_manual_add = gr.Button(value="Add folder")
            with gr.Column(scale=1):
                explorer_manual_remove = gr.Button(value="Remove selected")
        with gr.Row():
            with gr.Column(scale=2):
                explorer_year = gr.Dropdown(value="All", choices=shiro_year_choices(), label="Year", interactive=True)
            with gr.Column(scale=2):
                explorer_month = gr.Dropdown(value="All", choices=["All"] + [f"{i:02d}" for i in range(1, 13)], label="Month", interactive=True)
            with gr.Column(scale=2):
                explorer_day = gr.Dropdown(value="All", choices=["All"] + [f"{i:02d}" for i in range(1, 32)], label="Day", interactive=True)
            with gr.Column(scale=4):
                gr.HTML("<p class='no-gap-top mb-0 text-subdued'>Main filters by file modified date and scans limited subfolders. Raw shows the selected folder/subfolder directly without date filtering.</p>")
        ...
        with gr.Column():  
            with gr.Row():    
                ...
                with gr.Column(scale=1, elem_classes="right-column-panel"): 
                    ...
                    with gr.Box(visible=not others_dir) as exif_search_panel:
                        with gr.Row():
                    ...
                    with gr.Box(visible=not others_dir) as ranking_filter_panel:
                        with gr.Row():
                    ...
                    with gr.Box(visible=not others_dir) as aesthetic_score_filter_panel:
                        with gr.Row():
    ...
    # Shiro: keep New Favorite as a clean folder browser. Hide ranking,
    # EXIF keyword search and score/ranking filter widgets only in this tab.
    if others_dir:
        for _panel in (ranking_panel, exif_search_panel, ranking_filter_panel, aesthetic_score_filter_panel):
            _panel.visible = False

    change_dir_outputs = [warning_box, main_panel, img_path_browser, path_recorder, load_switch, img_path, img_path_depth]
    favorite_subdir_outputs = change_dir_outputs + [img_path_subdirs, img_path_subdirs_row, img_path_subdirs_level2, img_path_subdirs_level2_row, img_path_subdirs_level3, img_path_subdirs_level3_row]
    img_path.submit(shiro_favorite_textbox_change if others_dir else change_dir_textbox, inputs=[img_path, path_recorder, load_switch, img_path_browser, img_path_depth], outputs=(favorite_subdir_outputs if others_dir else change_dir_outputs), show_progress=show_progress_setting)
    img_path_browser.change(shiro_favorite_root_change if others_dir else change_dir_dropdown, inputs=[img_path_browser, path_recorder, load_switch, img_path_browser, img_path_depth], outputs=(favorite_subdir_outputs if others_dir else change_dir_outputs), show_progress=show_progress_setting)
    if others_dir:
        img_path_root_button.click(shiro_favorite_root_change, inputs=[img_path_browser, path_recorder, load_switch, img_path_browser, img_path_depth], outputs=favorite_subdir_outputs, show_progress=show_progress_setting)
        try:
            current_gr_tab.select(
                shiro_favorite_auto_open_once,
                inputs=[shiro_favorite_auto_opened, img_path_browser, path_recorder, load_switch, img_path_browser, img_path_depth],
                outputs=favorite_subdir_outputs + [shiro_favorite_auto_opened],
                show_progress=show_progress_setting
            )
        except Exception as e:
            logger.warning(f"Image Browser New Favorite: tab select auto Open root binding failed: {e}")
    ...
    explorer_source.change(
        shiro_explorer_change_source_with_subdirs,
        inputs=[explorer_source],
        outputs=[img_path, img_path_depth, img_path_subdirs, img_path_subdirs_row, img_path_subdirs_level2, img_path_subdirs_level2_row, img_path_subdirs_level3, img_path_subdirs_level3_row],
        show_progress=show_progress_setting
    )
    explorer_refresh.click(shiro_increment_page, inputs=[turn_page_switch], outputs=[page_index, turn_page_switch], show_progress=show_progress_setting)
    # Explorer filters intentionally do not auto-refresh; click Search / Refresh after changing them.
    explorer_manual_add.click(shiro_explorer_add_remove, inputs=[explorer_manual_path, path_recorder, img_path_add, explorer_manual_depth], outputs=[path_recorder, explorer_source], show_progress=show_progress_setting)
    explorer_manual_remove.click(shiro_explorer_add_remove, inputs=[explorer_source, path_recorder, img_path_remove, explorer_manual_depth], outputs=[path_recorder, explorer_source], show_progress=show_progress_setting)
    ...
    # New Favorite / Explorer subfolder navigation
    img_path_subdirs_button.click(
        fn=shiro_subdir_update,
        inputs=[img_path],
        outputs=[img_path_subdirs, img_path_subdirs_row],
        show_progress=show_progress_setting
    )
    img_path_subdirs_level2_button.click(
        fn=shiro_subdir_update,
        inputs=[img_path_subdirs],
        outputs=[img_path_subdirs_level2, img_path_subdirs_level2_row],
        show_progress=show_progress_setting
    )
    img_path_subdirs_level3_button.click(
        fn=shiro_subdir_update,
        inputs=[img_path_subdirs_level2],
        outputs=[img_path_subdirs_level3, img_path_subdirs_level3_row],
        show_progress=show_progress_setting
    )
    img_path_subdirs.change(
        fn=shiro_favorite_subdir_change,
        inputs=[img_path_subdirs, path_recorder, load_switch, img_path_browser, img_path_depth],
        outputs=change_dir_outputs + [img_path_subdirs_level2, img_path_subdirs_level2_row, img_path_subdirs_level3, img_path_subdirs_level3_row],
        show_progress=show_progress_setting
    )
    img_path_subdirs_level2.change(
        fn=shiro_favorite_subdir_change_level2,
        inputs=[img_path_subdirs_level2, path_recorder, load_switch, img_path_browser, img_path_depth],
        outputs=change_dir_outputs + [img_path_subdirs_level3, img_path_subdirs_level3_row],
        show_progress=show_progress_setting
    )
    img_path_subdirs_level3.change(
        fn=change_dir_dropdown,
        inputs=[img_path_subdirs_level3, path_recorder, load_switch, img_path_browser, img_path_depth],
